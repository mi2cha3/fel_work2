...
double	euler_implicit(double dt, double dx, double time, double *l_2,
		double *l_inf, FILE *fp_dist)
{
	...
	t_steps = round(time / dt);
	x_steps = round(M_PI / dx);
	r = g_kappa * dt / (dx * dx);
	rho = (2.0 * r / (1.0 + 2.0 * r)) * cos(M_PI / x_steps);
	omega = 2.0 / (1.0 + sqrt(1.0 - rho * rho));
	u = (double **)calloc(t_steps + 1, sizeof(double *));
	if (!u)
		exit(1);
	...
	for (int k = 0; k < t_steps; k++)
	{
		...
		while (max_error > tolerance && iter < 10000)
		{
			...
			for (int i = 1; i < x_steps; i++)
			{
				old_val = u[k + 1][i];
				// ガウス・ザイデルの仮更新値
				u_star = (u[k][i] + r * u[k + 1][i - 1] + r * u[k + 1][i + 1])
					/ (1.0 + 2.0 * r);
				// SOR法による実際の更新
				u[k + 1][i] = (1.0 - omega) * old_val + omega * u_star;
				current_error = fabs(u[k + 1][i] - old_val);
